...
def get_ideal_buy_sell_magic(option):
    option.IdealBuy = round(
        option.BidPrice + (option.CurrentSpread * .2), 3)
    option.IdealSell = round(
        option.AskPrice - (option.CurrentSpread * .2), 3)
...
if __name__ == '__main__':
    ...
    if len(eligibleOptions) == 0:
        ...
        for option in eligibleOptions:
            option.HistoricData["SMA"] = ta.sma(
                option.HistoricData["Close"], 5)
            option.HistoricData["RSI"] = ta.rsi(
                option.HistoricData["Close"], 5)
